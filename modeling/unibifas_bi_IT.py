...
class HierarchicalPromptLearner(nn.Module):
    ...
    Guidance Direction (bi_IT variant): V→T / T→V (No Bidirectional)
    ...
    def __init__(self, cfg, clip_model):
        ...
        # Only one boundary: V->T below, T->V above
    ...
    def _initialize_knowledge_proxies(self, ctx_dim, vis_dim, dtype):
        ...
        self.lkp_v_low = _get_clones(AttentionPooling(vis_dim, 8), self.transition_layer)
        self.lkp_t_high = _get_clones(AttentionPooling(ctx_dim, 8), self.prompt_depth - self.transition_layer)

        # Proxy tokens to act as queries in LKP
        self.v_proxy_low = nn.ParameterList([nn.Parameter(torch.randn(1, vis_dim, dtype=dtype)) for _ in range(self.transition_layer)])
        self.t_proxy_high = nn.ParameterList([nn.Parameter(torch.randn(1, ctx_dim, dtype=dtype)) for _ in range(self.prompt_depth - self.transition_layer)])
    ...
    def _initialize_postprocess_mlps(self, ctx_dim, vis_dim):
        ...
        self.visual_postprocess_mlp_low = nn.ModuleList([
            FeaturePreprocessMLP(vis_dim) for _ in range(self.transition_layer)
        ])
        self.text_postprocess_mlp_high = nn.ModuleList([
            FeaturePreprocessMLP(ctx_dim) for _ in range(self.prompt_depth - self.transition_layer)
        ])
    ...
    def forward(self):
        # --- Two-Stage Cross-Modal Interaction (No Bidirectional) ---

        # 1. Low Level: Vision -> Text (e.g., layers 0-6)
        proxy_v_tokens = []
        ...
        text_prompts_low = torch.cat([p.unsqueeze(0) for p in self.cross_prompts_text[:self.transition_layer]], dim=0)
        
        updated_text_low = self.v2t_low(text_prompts_low.flatten(0, 1), proxy_v_low, proxy_v_low)
        updated_text_low = updated_text_low.view(self.transition_layer, self.n_ctx, -1)
        
        for i in range(self.transition_layer):
            self.cross_prompts_text[i].data.copy_(updated_text_low[i])

        # 2. High Level: Text -> Vision (e.g., layers 7-11)
...
class UniBiFAS_Model(nn.Module):
    def __init__(self, cfg):
        ...
        # For bi_IT: only one interaction layer [7] -> index 6 (0-based)
        ...
        # CLS projections: For bi_IT, we only need 2 projections (transition + final)
    ...
    def forward(self, image):
        ...
        # For bi_IT: interaction_layers = [6] (transition), final_layer = 11
