...
class HierarchicalPromptLearner(nn.Module):
    ...
    Guidance Direction (TI variant): T→V / V↔T / V→T
    ...
    def _initialize_knowledge_proxies(self, ctx_dim, vis_dim, dtype):
        ...
        self.lkp_t_low = _get_clones(AttentionPooling(ctx_dim, 8), self.low_level_layers)
        ...
        self.lkp_v_high = _get_clones(AttentionPooling(vis_dim, 8), self.prompt_depth - self.mid_level_layers)

        # Proxy tokens to act as queries in LKP
        self.t_proxy_low = nn.ParameterList([nn.Parameter(torch.randn(1, ctx_dim, dtype=dtype)) for _ in range(self.low_level_layers)])
        ...
        self.v_proxy_high = nn.ParameterList([nn.Parameter(torch.randn(1, vis_dim, dtype=dtype)) for _ in range(self.prompt_depth - self.mid_level_layers)])
    ...
    def _initialize_postprocess_mlps(self, ctx_dim, vis_dim):
        ...
        self.text_postprocess_mlp_low = nn.ModuleList([
            FeaturePreprocessMLP(ctx_dim) for _ in range(self.low_level_layers)
        ])
        ...
        self.visual_postprocess_mlp_high = nn.ModuleList([
            FeaturePreprocessMLP(vis_dim) for _ in range(self.prompt_depth - self.mid_level_layers)
        ])
    ...
    def forward(self):
        ...
        # 1. Low Level: Text -> Vision (e.g., layers 0-3)
        proxy_t_tokens = []
        ...
        visual_prompts_low = torch.cat([p.unsqueeze(0) for p in self.cross_prompts_visual[:self.low_level_layers]], dim=0)
        
        updated_visual_low = self.t2v_low(visual_prompts_low.flatten(0, 1), proxy_t_low, proxy_t_low)
        updated_visual_low = updated_visual_low.view(self.low_level_layers, self.n_ctx, -1)
        ...
        # 3. High Level: Vision -> Text (e.g., layers 8-11)
