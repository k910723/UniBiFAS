...
class HierarchicalPromptLearner(nn.Module):
    ...
    def _initialize_attention_mechanisms(self, ctx_dim, vis_dim):
        ...
        # Vision -> Text (All Layers) - only one attention mechanism needed
        self.v2t = CrossPromptAttention(ctx_dim, vis_dim, num_attention_heads=8)

    def _initialize_knowledge_proxies(self, ctx_dim, vis_dim, dtype):
        """Initialize Layer-specific Knowledge Proxies (LKP)."""
        # Only visual proxies needed for V→T guidance across all layers
        self.lkp_v = _get_clones(AttentionPooling(vis_dim, 8), self.prompt_depth)

        # Proxy tokens to act as queries in LKP
        self.v_proxy = nn.ParameterList([nn.Parameter(torch.randn(1, vis_dim, dtype=dtype)) for _ in range(self.prompt_depth)])

    def _initialize_postprocess_mlps(self, ctx_dim, vis_dim):
        """Initialize MLP post-processing modules for LKP."""
        # Only visual postprocess MLPs needed for V→T guidance
        self.visual_postprocess_mlp = nn.ModuleList([
            FeaturePreprocessMLP(vis_dim) for _ in range(self.prompt_depth)
        ])
    ...
    def forward(self):
        # --- Unidirectional Vision → Text Guidance (All Layers) ---
        
        # Extract visual proxies from all layers using LKP
        proxy_v_tokens = []
        for i in range(self.prompt_depth):
            # Apply LKP first, then post-process with MLP
            proxy = self.lkp_v[i](self.v_proxy[i], self.cross_prompts_visual[i], self.cross_prompts_visual[i])
            proxy = self.visual_postprocess_mlp[i](proxy)
            proxy_v_tokens.append(proxy)
        
        # Stack all visual proxies
        proxy_v_all = torch.cat(proxy_v_tokens, dim=0)
        
        # Stack all text prompts
        text_prompts_all = torch.cat([p.unsqueeze(0) for p in self.cross_prompts_text], dim=0)
        
        # Update all text prompts using visual guidance
        updated_text_all = self.v2t(text_prompts_all.flatten(0, 1), proxy_v_all, proxy_v_all)
        updated_text_all = updated_text_all.view(self.prompt_depth, self.n_ctx, -1)
        
        # Update all text prompt parameters
        for i in range(self.prompt_depth):
            self.cross_prompts_text[i].data.copy_(updated_text_all[i])

        # Visual prompts remain unchanged (no T→V guidance)
        # This tests if T→V guidance is necessary at all
...
class UniBiFAS_Model(nn.Module):
    def __init__(self, cfg):
        ...
        # CLS projection: For uni_IT, only use final layer (high-level)
        self.final_cls_projection = ProjectionMLP(768, 512)
        
        # Define interaction layers for patch token extraction
        # For uni_IT: only extract from final layer for segmentation
    ...
    def _register_hooks(self):
        ...
        # For uni_IT, only register hook for final layer
    ...
    def forward(self, image):
        ...
        # Extract patch tokens from final layer only for uni_IT
