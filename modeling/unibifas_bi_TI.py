...
class HierarchicalPromptLearner(nn.Module):
    ...
    Guidance Direction (bi_TI variant): T→V / V→T (No Bidirectional, Inverted)
    ...
    def __init__(self, cfg, clip_model):
        ...
        # Only one boundary: T->V below, V->T above
        self.transition_layer = cfg['model']['interaction_layers'][0]
    ...
    def _initialize_attention_mechanisms(self, ctx_dim, vis_dim):
        ...
        # Text -> Vision (Low Level)
        self.t2v_low = CrossPromptAttention(vis_dim, ctx_dim, num_attention_heads=8)
        # Vision -> Text (High Level)
        self.v2t_high = CrossPromptAttention(ctx_dim, vis_dim, num_attention_heads=8)

    def _initialize_knowledge_proxies(self, ctx_dim, vis_dim, dtype):
        """Initialize Layer-specific Knowledge Proxies (LKP)."""
        self.lkp_t_low = _get_clones(AttentionPooling(ctx_dim, 8), self.transition_layer)
        self.lkp_v_high = _get_clones(AttentionPooling(vis_dim, 8), self.prompt_depth - self.transition_layer)

        # Proxy tokens to act as queries in LKP
        self.t_proxy_low = nn.ParameterList([nn.Parameter(torch.randn(1, ctx_dim, dtype=dtype)) for _ in range(self.transition_layer)])
        self.v_proxy_high = nn.ParameterList([nn.Parameter(torch.randn(1, vis_dim, dtype=dtype)) for _ in range(self.prompt_depth - self.transition_layer)])

    def _initialize_postprocess_mlps(self, ctx_dim, vis_dim):
        """Initialize MLP post-processing modules for LKP."""
        self.text_postprocess_mlp_low = nn.ModuleList([
            FeaturePreprocessMLP(ctx_dim) for _ in range(self.transition_layer)
        ])
        self.visual_postprocess_mlp_high = nn.ModuleList([
            FeaturePreprocessMLP(vis_dim) for _ in range(self.prompt_depth - self.transition_layer)
        ])
    ...
    def forward(self):
        # --- Two-Stage Cross-Modal Interaction (No Bidirectional, Inverted) ---

        # 1. Low Level: Text -> Vision (e.g., layers 0-6)
        proxy_t_tokens = []
        for i in range(self.transition_layer):
            # Apply LKP first, then post-process with MLP
            proxy = self.lkp_t_low[i](self.t_proxy_low[i], self.cross_prompts_text[i], self.cross_prompts_text[i])
            proxy = self.text_postprocess_mlp_low[i](proxy)
            proxy_t_tokens.append(proxy)
        
        proxy_t_low = torch.cat(proxy_t_tokens, dim=0)
        visual_prompts_low = torch.cat([p.unsqueeze(0) for p in self.cross_prompts_visual[:self.transition_layer]], dim=0)
        
        updated_visual_low = self.t2v_low(visual_prompts_low.flatten(0, 1), proxy_t_low, proxy_t_low)
        updated_visual_low = updated_visual_low.view(self.transition_layer, self.n_ctx, -1)
        
        for i in range(self.transition_layer):
            self.cross_prompts_visual[i].data.copy_(updated_visual_low[i])

        # 2. High Level: Vision -> Text (e.g., layers 7-11)
        high_range = range(self.transition_layer, self.prompt_depth)
        proxy_v_tokens_high = []
        for i, layer_idx in enumerate(high_range):
            # Apply LKP first, then post-process with MLP
            proxy = self.lkp_v_high[i](self.v_proxy_high[i], self.cross_prompts_visual[layer_idx], self.cross_prompts_visual[layer_idx])
            proxy = self.visual_postprocess_mlp_high[i](proxy)
            proxy_v_tokens_high.append(proxy)
            
        proxy_v_high = torch.cat(proxy_v_tokens_high, dim=0)
        text_prompts_high = torch.cat([self.cross_prompts_text[i].unsqueeze(0) for i in high_range], dim=0)
        
        updated_text_high = self.v2t_high(text_prompts_high.flatten(0,1), proxy_v_high, proxy_v_high).view_as(text_prompts_high)

        for i, layer_idx in enumerate(high_range):
            self.cross_prompts_text[layer_idx].data.copy_(updated_text_high[i])
...
class UniBiFAS_Model(nn.Module):
    def __init__(self, cfg):
        ...
        # For bi_TI: only one interaction layer [7] -> index 6 (0-based)
        self.interaction_layers = [layer - 1 for layer in cfg['model']['interaction_layers']]  # [6] (0-based)
        self.final_layer = cfg['model']['prompt_depth'] - 1  # 11 for ViT-B/16
        
        # CLS projections: For bi_TI, we only need 2 projections (transition + final)
        self.transition_cls_projection = ProjectionMLP(768, 512)
    ...
    def forward(self, image):
        ...
        # Extract patch tokens from transition and final layers for segmentation
        # For bi_TI: interaction_layers = [6] (transition), final_layer = 11
        target_layers = self.interaction_layers + [self.final_layer]  # [6, 11]
        ...
        # Aggregate patch tokens from transition and final layers
        ...
        # Get cls tokens from transition and final layers
        cls_tokens_list = []
        for layer_idx in target_layers:
        ...
        # Stack cls tokens from transition and final layers (2 tokens total)
        cls_tokens = torch.cat(cls_tokens_list, dim=1)  # [batch, 2, 768]
        ...
        cls_tokens_projected[:,0,:] = self.transition_cls_projection(cls_tokens[:,0,:])
        cls_tokens_projected[:,1,:] = self.final_cls_projection(cls_tokens[:,1,:])
